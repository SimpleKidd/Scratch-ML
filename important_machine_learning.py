# -*- coding: utf-8 -*-
"""Important_Machine_Learning.ipynb
...
# class lda:
#   def __init__(self,conserved_variance = None,k = None):
#     self.threshold = conserved_variance
#     self.k = k
  
#   def within_class(self,X,y):
#     classes = np.unique(y)
#     Sw = []
#     flag = 0
#     for i in classes:
...
#       Xi = X[y==i]
#       mean = np.mean(Xi,axis = 0)
#       diff = np.array(mean - total_mean).reshape(-1,1)

#       if flag == 0:
#         Sb = len(Xi)*(diff @ diff.T)
#         flag = 1
#       else:
#         Sb += len(Xi)*(diff @ diff.T)
#     return Sb
...
#   def fit(self,X,y):
#     self.Sw = self.within_class(X,y)
#     self.Sb = self.between_class(X,y)

#     se = np.linalg.inv(self.Sw)
#     S = np.linalg.inv(self.Sw) @ self.Sb
#     eigen_values, eigen_vectors = np.linalg.eig(S)

#     indexs = np.argsort(eigen_values)[::-1]
    
#     eigen_values = np.array([eigen_values[i] for i in indexs])
#     eigen_vectors = np.array([eigen_vectors[i] for i in indexs]).real
#     self.features_order = np.array(indexs)

#     self.total_eigval = sum(eigen_values)

#     self.number_of_linear_discriminants = 1
#     if self.k == None:
#       for i in range(len(eigen_values)):
#         if sum(eigen_values[:i+1])/self.total_eigval >= self.threshold:
#           self.number_of_linear_discriminants = i+1
#           break
#       else:
#         self.number_of_linear_discriminants = X.shape[1]
#     else:
#       self.number_of_linear_discriminants = self.k
#     self.features_order = self.features_order[:self.number_of_linear_discriminants]
#     self.eigen_values = eigen_values[:self.number_of_linear_discriminants]
#     self.eigen_vectors = eigen_vectors[:self.number_of_linear_discriminants]

#     self.model = Gaussian_Classifier(3)
#     self.model.train(X,y)
#     return self.number_of_linear_discriminants

#   def transform(self,X):
#     return X.dot(self.eigen_vectors.T)

#   def test(self,X,y):
#     X = self.transform(X)
#     p,a = self.model.test(X,y)
#     return p,a